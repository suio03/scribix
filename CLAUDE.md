...
Paddle is wired through `app/api/paddle/create-checkout/route.ts`, `app/api/paddle/create-portal/route.ts`, and `app/api/webhook/paddle/route.ts`. The localized `/pricing` page displays Free, Starter, and Pro; public amounts and allowances come from `lib/pricing-v2.ts` and `lib/plans.ts`. New checkout requires `version: "v2"` and `PADDLE_V2_CHECKOUT_ENABLED=true` (enabled in production on 2026-09-23); setting the flag to false is the rollback switch and makes the page mark paid checkout as unavailable. Legacy price IDs remain configured only to recognize existing subscriptions and grant their original entitlements; never offer them in new checkout. `/dashboard/billing` manages the current subscription, usage, and Paddle portal. Paddle is the source of truth for actual monetary amounts. Scribix stores transaction and adjustment ownership metadata for entitlement, idempotency, and routing, but does not copy revenue, tax, or refund amounts. The webhook verifies Paddle signatures, dedupes events in `paddle_events`, stores Paddle customer/subscription IDs on `users`, resets minute and YouTube-import counters when a billing period advances, and expires users back to free when paid access ends. Migration `0043_pricing_plan_version.sql` must exist in any remote environment before this code runs (applied in production on 2026-09-23).
