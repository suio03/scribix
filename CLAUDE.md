...
Quota model (`lib/quota.ts` + `lib/plans.ts`): a **single `minutes_used_this_period` counter** on `users`, no credit ledger. Free transcription minutes are a lifetime trial defined in `lib/plans.ts` and never reset. Free file uploads may be up to 10 hours, but only the explicitly confirmed remaining minutes are processed; this strict consent policy applies to `source="upload"` only. Paid tiers and recording paths retain the legacy half-estimate threshold until their UX is redesigned. New v2 Starter (`basic`) and Pro (`pro`) subscriptions receive the monthly `V2_PLANS` allowance on both billing cycles; legacy subscriptions keep their original allowance (legacy Creator: 2,400 minutes per month). Yearly plans are billed annually, but `lib/quota-period.ts` derives monthly allowance windows from the Paddle annual period and lazily resets usage with no rollover. Reservations are atomic and settlement never exceeds `reserved_minutes`; work started in an older allowance window cannot change the current window's usage.
...
Paddle is wired through `app/api/paddle/create-checkout/route.ts`, `app/api/paddle/create-portal/route.ts`, and `app/api/webhook/paddle/route.ts`. The localized `/pricing` page displays Free, Starter, and Pro; public amounts and allowances come from `lib/pricing-v2.ts` and `lib/plans.ts`. New checkout requires `version: "v2"` and `PADDLE_V2_CHECKOUT_ENABLED=true` (enabled in production on 2026-09-23); setting the flag to false is the rollback switch and makes the page mark paid checkout as unavailable. Legacy price IDs remain configured only to recognize existing subscriptions and grant their original entitlements; never offer them in new checkout. `/dashboard/billing` manages the current subscription, usage, and Paddle portal. Paddle is the source of truth for actual monetary amounts. Scribix stores transaction and adjustment ownership metadata for entitlement, idempotency, and routing, but does not copy revenue, tax, or refund amounts. The webhook verifies Paddle signatures, ignores Muzix events from the shared Paddle account (`SIBLING_PADDLE_PRICE_PROJECTS` in `lib/paddle-webhook-routing.ts`), dedupes events in `paddle_events`, stores Paddle customer/subscription IDs on `users`, resets minute and YouTube-import counters when a billing period advances, and expires users back to free when paid access ends, including effective `subscription.canceled`/`paused` events after refunds or failed renewals. Migration `0043_pricing_plan_version.sql` must exist in any remote environment before this code runs (applied in production on 2026-09-23).
...
**AI Notes** is the user-facing name for the existing summary pipeline; internal routes, storage fields, and types may still use `summary`. Paid users (every non-free tier) generate notes from a completed transcript through `GET/POST /api/transcripts/[id]/summary`; both methods reject Free users. Translation read and generation routes enforce the same paid-only boundary. `lib/openai-summary.ts` asks OpenAI for an overview, key points, and concrete action items, then the route stores the payload in R2 and status metadata in D1. `OPENAI_API_KEY` is required.
...
Free and legacy Starter (`basic`) accounts receive 3 successful questions for the account lifetime; v2 Starter, v2 Pro and legacy Pro/Creator receive a per-period allowance from `aiQuestionsFor` in `lib/plans.ts`. Quota increments are conditional and provider failures refund the increment. Chat messages persist in `ai_chat_messages`; clearing chat does not refund quota, while transcript/account deletion hard-deletes chat content. `ai_usage_events` separately retains token counts, cached input, per-token prices, and estimated cost; transcript/account deletion nulls identifying foreign keys rather than deleting accounting history. Migrations `0022` through `0024` define this schema. Anonymous `ask_ai_*` product events in `lib/analytics.ts` cover submitted questions, outcomes, quota, upgrades, and clearing with plan/question/transcript-source and truncation/error metadata only—never IDs, titles, questions, answers, or transcript content. The transcript workspace defaults to Ask AI beside the transcript, with AI Notes as the alternate panel and Export in a modal.
...
`app/sitemap.ts` emits nine core public routes (home, four transcription tools, AI Note Taker, pricing, `/long-video-to-short-video-ai`, and `/youtube-shorts-maker`) plus `/guides`, `/podcast-clip-maker`, and every guide in all six locales; three English-only legal pages; and English-only `/alternatives`, alternative articles, and `/partners`. Every localized entry includes the full language-alternate set. `lib/metadata-url.ts` supplies URL helpers and the shared social image; route metadata must retain self-canonical URLs, reciprocal hreflang, and page-specific Open Graph URLs. The organization logo uses `/brand/scribix.svg`; the shared social image is `/brand/social.png`. Legacy `/opengraph-image` routes redirect to that static PNG because dynamic image rendering returned 500 on the deployed OpenNext Worker. `Footer.tsx` exposes ordinary localized links to all five transcription tools and the long-video, YouTube Shorts and podcast clipping pages. The long-video landing page also links directly to `/youtube-shorts-maker`. The Shorts page uses a single localized H1 containing its category label, shared social imagery, and WebPage JSON-LD; see [Shorts page implementation and media](docs/landing-page-youtube-shorts-maker.md) for the fresh demo source and localized render command.
...
- Deploys: pushing `main` triggers Cloudflare Workers Builds for the app (the ClipFlight/Teleo sibling repo deploys the same way). Apply remote D1 migrations before pushing schema-dependent code; do not run `npm run deploy` unless the owner asks for a manual deploy.
- Commits: short imperative, Conventional Commit prefixes (`feat:`, `fix:`). `CHANGELOG.md` is maintained per release.

## Reference

Start with [docs/README.md](docs/README.md) for current contracts, operations, research, and future proposals. `docs/archive/` retains referenced design studies, acceptance criteria, and unique POC measurements. Superseded implementation plans are removed; use Git history for earlier versions. Historical checkboxes and deployment statements are not current evidence. Ask AI behavior and outstanding smoke checks live in `docs/transcript-ask-ai.md`; video contracts and external release checks live in `docs/video-workspace/`. Collections and Ask AI v2 remain proposals in `docs/roadmap/collections-transcript-ai.md`.
