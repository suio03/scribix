...
发布记录：`0cf1dc2` 已推送 `origin/main`，2026-09-19 经 `npm run deploy` 部署 Cloudflare（初次版本 `92e68a58-8037-406a-b041-3afd2d5ee670`）。线上英语教程已通过浏览器验证；补充清理六语言视频说明残留的“本地预览”措辞。原始字幕不变。

最终生产版本：`229bd146-643b-4095-bc07-da62983fb18b`。线上 24 个内容页面及六语言教程 sitemap 已验证；七个 MP4 均可下载，六语言视频说明与最终文案一致。Chrome 验证 41 秒演示和 57.43 秒真实导出均能播放。当前媒体 Range 请求返回完整文件，尚未确认分段传输；不影响本次浏览器播放验收。
