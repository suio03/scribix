...
本地实现，未确认生产部署。2026-09-15 已用真实新项目走通上传至成片生成；实际下载由用户验收，首期尚未整体完成。产品范围见[首期产品计划](../archive/plans/publish-preparation-design.md)。登录后真实上传至成片生成已验证；实际下载及无指导新手任务仍待验收。

## 选片

2026-09-17 新增默认关闭的后台批量链路，详见 [第一阶段方案](../roadmap/ai-clips-generation-workflow.md)与[验证记录](ai-analysis-validation.md)。开启后 POST 返回 202，等待转写、发现、复审和恢复均由后台负责；支持明确的最多 180 分钟分析范围。下文请求内执行及十分钟失效规则仅适用于关闭开关后的历史链路。
