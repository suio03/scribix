...
本次本地结果：24/24 通过，6 个源 profile，累计渲染 10,432ms，单 case p50 423ms、p95 501ms、最大 523ms。合成视频只有 1.2 秒，用于回归正确性，不代表 AWS Batch 的生产延迟或真实内容质量。
...
- ≤180 秒输出的 p95 total latency ≤ 10 分钟，p95 start latency ≤ 5 分钟。
- 成功 render 的唯一下载率 ≥ 70%。
...
- 获得参与者明确同意，并说明源视频保留期、第三方转录与 AWS 处理位置。
- 每位用户完成候选选择、编辑、至少一次成片、下载，并回答是否仍需 CapCut/Premiere 以及原因。
- 每日查看管理员指标、dispatcher metrics、DLQ、cleanup、Batch/ECR 告警和成本。
