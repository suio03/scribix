...
export async function PATCH(request: Request, { params }: Params) {
  ...
  if (!session) return Response.json({ error: "unauthorized" }, { status: 401 });
  const { id: projectId, candidateId } = await params;
  const body = await request.json<{ title?: unknown; reviewMark?: unknown }>().catch(() => null);
  const title = typeof body?.title === "string"
    ? body.title.trim().replace(/\s+/g, " ").slice(0, 160)
    : "";
  const marking = body && Object.hasOwn(body, "reviewMark");
  if (marking && body.reviewMark !== null && !["keep", "discard"].includes(String(body.reviewMark))) return Response.json({error:"invalid_review_mark"},{status:400});
  if (!marking && !title) return Response.json({ error: "invalid_title" }, { status: 400 });

  const env = await cf();
  const user = await getOrCreateCurrentUser(env.DB, session);
  if (!user) return Response.json({ error: "user_not_found" }, { status: 404 });
  if (marking) {
    const result = await env.DB.prepare("UPDATE clip_candidates SET review_mark=?1 WHERE id=?2 AND project_id=?3 AND user_id=?4 AND status<>'deleted' AND EXISTS(SELECT 1 FROM video_projects WHERE id=?3 AND user_id=?4 AND deleted_at IS NULL)").bind(body!.reviewMark, candidateId, projectId, user.id).run();
    return result.meta?.changes ? Response.json({ok:true,reviewMark:body!.reviewMark}) : Response.json({error:"candidate_not_found"},{status:404});
  }
  if (!videoWorkspaceAccessFor(user.tier).canEditClips) {
    return Response.json({ error: "upgrade_required" }, { status: 402 });
  }
