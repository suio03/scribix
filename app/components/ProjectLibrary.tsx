...
export async function ProjectLibrary({
  ...
  const nav = await getTranslations("WorkspaceNav");
  const Heading = home ? "h2" : "h1";
  const uploadPath = getPathname({href: "/dashboard", locale});
  ...
  return (
    <main className="product-surface-refresh mx-auto max-w-[1120px] px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
      ...
      {home && <section className="mb-12">
        <h1 className="font-display text-3xl font-semibold tracking-tight text-ink sm:text-4xl">{nav("headline")}</h1>
        <p className="mt-3 text-muted">{nav("intro")}</p>
        <div className="dashboard-create-panel workspace-upload-surface mt-6 overflow-hidden rounded-3xl border border-line bg-card p-3">
          <Uploader signedIn postSignInPath={uploadPath} checkoutSuccessPath={uploadPath} tier={user.tier} toolSlug="dashboard-clips" videoOnly />
        </div>
      </section>}
      <div className="flex items-end justify-between gap-5">
        <div>
          ...
          <Heading className="font-display text-[30px] font-semibold tracking-[-0.035em] text-ink sm:text-[36px]">
            {home ? nav("recent") : nav("projects")}
          </Heading>
          <p className="mt-1.5 text-[13px] leading-6 text-muted">{home ? "" : t("description")}</p>
        </div>
        <Link
          href={home ? "/dashboard/projects" : "/dashboard/new"}
        ...
        >
          {home ? <ArrowUpRight size={14} /> : <Plus size={14} />}
          {home ? nav("allProjects") : nav("newProject")}
        </Link>
      </div>
              ...
              >
                <div className="flex h-full flex-col">
                  <Link
                    ...
                    className="relative block aspect-video w-full shrink-0 overflow-hidden border-b border-line bg-[#17102f]"
                  >
                  ...
                  <div className="flex min-w-0 flex-1 flex-col justify-center p-4 sm:px-5">
                    <div className="flex min-w-0 flex-wrap items-start gap-2">
                      <div className="min-w-0 w-full">
                        ...
                        <Link
                          href={href}
                          className="mt-1.5 block line-clamp-2 text-[15px] font-semibold tracking-[-0.01em] text-ink transition group-hover:text-accent sm:text-[16px]"
                        >
                          {row.title}
                        </Link>
                      </div>
                    ...
                    <div className="mt-4 flex items-center justify-between gap-4">
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[12px] text-muted">
                        ...
                        {!home && row.source_available === 1 && row.source_expires_at ? (
                          <>
                            <span aria-hidden>·</span>
