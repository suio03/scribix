...
const WORKFLOW = [
  ...
  { key: "discover", icon: WandSparkles },
  { key: "edit", icon: Scissors },
  { key: "export", icon: Download },
] as const satisfies ReadonlyArray<{ key: string; icon: LucideIcon }>;

const FEATURES = [
  { key: "candidates", icon: Sparkles },
  { key: "timeline", icon: Film },
  { key: "captions", icon: Captions },
  { key: "crop", icon: Crop },
  { key: "brand", icon: Palette },
  { key: "render", icon: Download },
] as const satisfies ReadonlyArray<{ key: string; icon: LucideIcon }>;
...
export async function VideoHomeMarketing() {
  ...
  const workflow = mergeLocalizedItems(
    t.raw("workflow.steps") as ItemCopy[],
    WORKFLOW,
    "VideoHome.workflow.steps"
  );
  const features = mergeLocalizedItems(
    t.raw("features.items") as ItemCopy[],
    FEATURES,
    "VideoHome.features.items"
  );
      ...
      <section id="video-how" className="scroll-mt-20 border-y border-line bg-card/55 px-4 py-20 sm:px-8 sm:py-28">
        <div className="mx-auto max-w-[1180px]">
          ...
          <div className="grid gap-8 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] xl:items-end">
            ...
            <p className="max-w-[58ch] text-[16px] leading-[1.75] text-muted lg:justify-self-end">
              {t("workflow.intro")}
            </p>
          </div>

          <div className="home-workflow-grid mt-14 grid gap-8 md:grid-cols-2 xl:grid-cols-4 xl:gap-0">
            {workflow.map((step, index) => {
              const Icon = step.icon;
              return (
                <article key={step.key} className="home-workflow-step relative border-l border-line py-3 pl-6 pr-6 lg:min-h-[16rem] lg:py-0 lg:pl-7">
                  <div className="mb-10 flex items-center justify-between">
                    <span className="font-mono text-[11px] font-semibold uppercase tracking-[0.14em] text-accent">
                      {String(index + 1).padStart(2, "0")}
                    </span>
                    <Icon size={22} strokeWidth={1.45} className="text-muted" />
                  </div>
                  <h3 className="font-display text-[23px] font-semibold leading-[1.18] tracking-[-0.025em]">
                    {step.title}
                  </h3>
                  <p className="mt-4 text-[14px] leading-[1.7] text-muted">{step.body}</p>
                </article>
              );
            })}
          </div>
          ...
          <div className="mt-14 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {features.map((feature, index) => {
              ...
              return (
                <article
                  key={feature.key}
                  className="home-feature-card group relative min-h-[15rem] overflow-hidden rounded-2xl border border-line bg-card p-7"
                >
                  <span className="absolute right-5 top-4 font-mono text-[10px] tracking-[0.14em] text-muted/55">
                    {String(index + 1).padStart(2, "0")}
                  </span>
                  <span className="inline-grid size-11 place-items-center rounded-xl bg-accent-soft text-accent transition group-hover:rotate-[-3deg] group-hover:scale-105">
                    <Icon size={19} strokeWidth={1.55} />
                  </span>
                  <h3 className="mt-8 font-display text-[22px] font-semibold tracking-[-0.025em]">
                    {feature.title}
                  </h3>
            ...
            {audiences.map((audience) => {
              ...
              return (
                <article key={audience.key} className="home-audience-card bg-ink p-7 transition hover:bg-paper/[0.035]">
                  ...
                  <h3 className="mt-9 font-display text-[23px] font-semibold tracking-[-0.025em]">
                    {audience.title}
                  </h3>
                  <p className="mt-3 text-[14px] leading-[1.7] text-paper/55">{audience.body}</p>
                </article>
              );
            })}
          </div>
      ...
      <section className="home-secondary-tools border-y border-line bg-card/55 px-4 py-7 sm:px-8">
        <div className="mx-auto flex max-w-[1180px] flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[14px] text-muted">{t("secondaryTools.label")}</p>
          </div>
          <div className="flex flex-wrap items-center gap-4 text-[13px] font-semibold">
            <Link href="/audio-to-text" className="underline decoration-line underline-offset-4 transition hover:decoration-accent">
              {t("secondaryTools.audio")}
            </Link>
            <Link href="/youtube-to-transcript" className="underline decoration-line underline-offset-4 transition hover:decoration-accent">
              {t("secondaryTools.youtube")}
            </Link>
            <Link href="/ai-note-taker" className="underline decoration-line underline-offset-4 transition hover:decoration-accent">
        ...
        <div className="mx-auto grid max-w-[1180px] gap-12 lg:grid-cols-[minmax(0,0.75fr)_minmax(0,1.25fr)]">
          <div className="lg:sticky lg:top-24 lg:self-start">
            ...
            <p className="mt-5 max-w-[36ch] text-[14px] leading-[1.7] text-muted">
              {t("faq.contactPrefix")} {" "}
              <a href="mailto:hello@scribix.io" className="font-semibold text-ink underline decoration-accent decoration-2 underline-offset-4">
                hello@scribix.io
              </a>
            </p>
          </div>
            ...
            {faqs.map((faq, index) => (
              <details key={faq.q} className="group py-6">
                <summary className="flex items-start gap-5">
                  ...
                  <span className="faq-icon text-xl font-light leading-none text-muted transition">+</span>
                </summary>
      ...
      <section className="px-4 pb-20 sm:px-8">
        <div className="home-video-final relative mx-auto max-w-[1180px] overflow-hidden rounded-[1.75rem] border border-line bg-card px-6 py-14 sm:px-12 sm:py-20">
          ...
          <div className="relative max-w-[760px]">
            ...
            <a href="#video-upload" className="mt-8 inline-flex items-center gap-2 rounded-full bg-ink px-6 py-3.5 text-[14px] font-semibold text-paper transition hover:-translate-y-0.5 hover:bg-accent">
              {t("final.cta")}
