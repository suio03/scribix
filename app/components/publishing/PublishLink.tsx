...
export function PublishLink({ projectId, candidateId, disabled }: {
  ...
  if (upgrade) return <UpgradePlanButton checkoutSuccessPath={`/dashboard/video-projects/${projectId}`} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-line px-4 text-sm font-semibold text-accent"><Send size={16} />{socialT("upgrade")}</UpgradePlanButton>;
