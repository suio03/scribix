...
export function SocialWorkspacePreview({view}: {view: SocialPreviewView}) {
  ...
  const checkoutSuccessPath = view === "channels" ? "/dashboard/accounts" : view === "planner" ? "/dashboard/planner" : "/dashboard/publishing";
  ...
  return <>
    <header className="mb-6 flex items-center justify-between gap-4"><h1 className="font-display text-2xl font-semibold">{nav(view)}</h1><UpgradePlanButton checkoutSuccessPath={checkoutSuccessPath} className={outlineClass}><LockKeyhole size={15}/>{t("unlock")}</UpgradePlanButton></header>
    <section className="relative overflow-hidden rounded-2xl border border-accent/25 bg-accent/10 p-6 sm:p-8">
      <div className="grid items-center gap-8 lg:grid-cols-[1.4fr_1fr]">
        <div><h2 className="max-w-xl font-display text-2xl font-semibold tracking-tight sm:text-3xl">{t("workspaceTitle")}</h2>
          ...
          <UpgradePlanButton checkoutSuccessPath={checkoutSuccessPath} className={upgradeClass}><LockKeyhole size={16}/>{t("unlock")}</UpgradePlanButton>
        </div>
    ...
    <section className="mt-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3"><h2 className="font-display text-lg font-semibold">{nav(view)}</h2><UpgradePlanButton checkoutSuccessPath={checkoutSuccessPath} className={outlineClass}><Plus size={16}/>{t(view==="channels"?"connect":view==="planner"?"schedulePost":"newPost")}</UpgradePlanButton></div>
