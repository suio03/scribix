...
export async function maybeResetFreePeriod(db: D1Database, user: UserQuotaRow): Promise<UserQuotaRow> {
  ...
  const now = new Date();
  const ends = new Date(user.period_ends_at.replace(" ", "T") + "Z");
  ...
  const refreshed = await db
    .prepare(
      `SELECT id, tier, minutes_used_this_period, period_started_at, period_ends_at
...
export async function reserveQuota(
  ...
  const cap = PLANS[fresh.tier].minutesPerCycle;
